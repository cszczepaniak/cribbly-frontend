...
describe('auth reducer', () => {
    ...
    test('sign in request sets isSignedIn to false', () => {
        const state = generateState(
            authReducer,
            AuthActions.signInSuccess(ModelFactory.createUser()),
    ...
    test('sign in success loads the user into redux', () => {
        ...
        const state = generateState(authReducer, AuthActions.signInSuccess(user));

        expect(state.user).toEqual(user);
    });
    test('sign in success sets isSignedIn to true', () => {
        const state = generateState(authReducer, AuthActions.signInSuccess(ModelFactory.createUser()));
